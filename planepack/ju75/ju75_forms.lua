...
function ju75.getPlaneFromPlayer(player)
...
function ju75.pax_formspec(name)
    ...
    minetest.show_formspec(name, "ju75:passenger_main", basic_form)
end

function ju75.pilot_formspec(name)
    local basic_form = table.concat({
        "formspec_version[5]",
        "size[6,12]",
	}, "")
    ...
    local plane_obj = ju75.getPlaneFromPlayer(player)
    ...
    minetest.show_formspec(name, "ju75:pilot_main", basic_form)
end

function ju75.paint_formspec(name)
    local basic_form = table.concat({
        "formspec_version[3]",
        "size[4.0, 4.2]",
    },"")

    basic_form = basic_form.."image_button[0.5,0.5;3,1;ju75_p_lufthansa.png;lufthansa;Lufthansa;false;true;]"
    basic_form = basic_form.."image_button[0.5,1.6;3,1;ju75_p_lufthansa.png;lufthansa2;Lufthansa 2;false;true;]"
    basic_form = basic_form.."image_button[0.5,2.7;3,1;ju75_p_luftwaffe.png;luftwaffe;Luftwaffe;false;true;]"
    --basic_form = basic_form.."image_button[1,4.3;3,1;ju75_white.png^[multiply:#2b2b2b;black;Black;false;true;]" 

    minetest.show_formspec(name, "ju75:paint", basic_form)
end

minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "ju75:paint" then
        local name = player:get_player_name()
        local plane_obj = ju75.getPlaneFromPlayer(player)
        if plane_obj == nil then
            minetest.close_formspec(name, "ju75:paint")
            return
        end
        local ent = plane_obj:get_luaentity()
        if ent then
            ent.initial_properties.textures = ju75.textures_copy() --reset the textures first
            local search_string = ju75.skin_texture --then set to find the defaults

            if fields.lufthansa then ju75.set_skin(plane_obj, "ju75_skin_lufthansa.png", search_string) end
            if fields.lufthansa2 then ju75.set_skin(plane_obj, "ju75_skin_lufthansa2.png", search_string) end
            if fields.luftwaffe then ju75.set_skin(plane_obj, "ju75_skin_luftwaffe.png", search_string) end
        end
        minetest.close_formspec(name, "ju75:paint")
	end
	if formname == "ju75:passenger_main" then
        local name = player:get_player_name()
        local plane_obj = ju75.getPlaneFromPlayer(player)
        if plane_obj == nil then
            minetest.close_formspec(name, "ju75:passenger_main")
            return
        end
        local ent = plane_obj:get_luaentity()
        if ent then
		    if fields.new_seat then
                ju75.dettach_pax(ent, player)
                ju75.attach_pax(ent, player)
		    end
    ...
    if formname == "ju75:pilot_main" then
        local name = player:get_player_name()
        local plane_obj = ju75.getPlaneFromPlayer(player)
        if plane_obj == nil then
            minetest.close_formspec(name, "ju75:pilot_main")
            return
        end
        local ent = plane_obj:get_luaentity()
        if ent then
		    if fields.turn_on then
                ju75.start_engine(ent)
		    end
            if fields.hud then
		    ...
		    if fields.go_out then
                ...
                if ent.isinliquid or touching_ground then --isn't flying?
                    ...
                    if ent._passenger then
                        ...
                        if passenger then ju75.dettach_pax(ent, passenger) end
                    end
                    for i = 10,1,-1 
                    do 
                        if ent._passengers[i] then
                            ...
                            if passenger then
                                ju75.dettach_pax(ent, passenger)
		    ...
		    if fields.copilot then
                ...
                if ent._passenger and is_there_a_free_seat then
                    ...
                    if copilot_player_obj then
                        ju75.dettach_pax(ent, copilot_player_obj)
                        ju75.attach_pax(ent, copilot_player_obj)
                    else
                        ent._passenger = nil
                    end
                end
                ...
                if ent._passenger == nil then
                    ...
                    if new_copilot_player_obj then
                        ju75.dettach_pax(ent, new_copilot_player_obj)
                        ju75.attach_pax(ent, new_copilot_player_obj, true)
                    end
                end
		    end
            if fields.adf then

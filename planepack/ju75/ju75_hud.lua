...
function ju75.animate_gauge(player, ids, prefix, x, y, angle)
...
function ju75.update_hud(player, climb, speed, power, fuel)
    ...
    local ids = ju75.hud_list[player_name]
    if ids then
        ju75.animate_gauge(player, ids, "clb_pt_", clb_gauge_x, clb_gauge_y, climb)
        ju75.animate_gauge(player, ids, "sp_pt_", sp_gauge_x, sp_gauge_y, speed)
        ju75.animate_gauge(player, ids, "pwr_pt_", pwr_gauge_x, pwr_gauge_y, power)
        ju75.animate_gauge(player, ids, "fu_pt_", fu_gauge_x, fu_gauge_y, fuel)
    else
        ids = {}
        ...
        ju75.hud_list[player_name] = ids
    end
end


function ju75.remove_hud(player)
    if player then
        local player_name = player:get_player_name()

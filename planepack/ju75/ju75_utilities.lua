...
dofile(minetest.get_modpath("ju75") .. DIR_DELIM .. "ju75_hud.lua")

function ju75.get_hipotenuse_value(point1, point2)
    return math.sqrt((point1.x - point2.x) ^ 2 + (point1.y - point2.y) ^ 2 + (point1.z - point2.z) ^ 2)
end

function ju75.dot(v1,v2)
	return v1.x*v2.x+v1.y*v2.y+v1.z*v2.z
end

function ju75.sign(n)
	return n>=0 and 1 or -1
end

function ju75.minmax(v,m)
	return math.min(math.abs(v),m)*ju75.sign(v)
end

function ju75.get_gauge_angle(value, initial_angle)
...
-- attach player
function ju75.attach(self, player)
...
function ju75.dettachPlayer(self, player)
    local name = self.driver_name
    airutils.setText(self, "Ju 75")
...
function ju75.check_passenger_is_attached(self, name)
...
-- attach passenger
function ju75.attach_pax(self, player, is_copilot)
...
function ju75.dettach_pax(self, player)
...
function ju75.checkAttach(self, player)
    if player then
...
--painting
function ju75.paint(object, colstr, search_string)
    if colstr then
        local entity = object:get_luaentity()
        entity._color = colstr
        entity._skin = ju75.skin_texture
        local l_textures = ju75.textures_copy()
...
function ju75.set_skin(object, skin_image_name, search_string)
...
function ju75.start_engine(self)
    if self._engine_running then
	    self._engine_running = false
        -- sound and animation
        if self.sound_handle then
    ...
    elseif self._engine_running == false and self._energy > 0 then
        ...
        -- sound and animation
        ju75.engineSoundPlay(self)
        self.engine:set_animation_frame_speed(60)
    end
end

-- destroy the boat
function ju75.destroy(self)
    if self._engine_running then
	    self._engine_running = false
        self.engine:set_animation_frame_speed(0)
    end
    ...
    if self.driver_name then
        ...
        ju75.dettachPlayer(self, player)
    end
    ...
    self.object:remove()
    if not minetest.settings:get_bool('ju75.disable_craftitems') then
        pos.y=pos.y+2
        minetest.add_item({x=pos.x+math.random()-0.5,y=pos.y,z=pos.z+math.random()-0.5},'ju75:wings')
        ...
        minetest.add_item({x=pos.x+math.random()-0.5,y=pos.y,z=pos.z+math.random()-0.5},'ju75:ju75')
    end
end

function ju75.testDamage(self, velocity, position)
    ...
    local impact = abs(ju75.get_hipotenuse_value(velocity, self._last_vel))
    ...
    if impact > 1 and self._longit_speed > 2 then
        local noded = airutils.nodeatpos(airutils.pos_shift(p,{y=-2.8}))
	    if (noded and noded.drawtype ~= 'airlike') then
            minetest.sound_play("ju75_touch", {
    ...
    if self._longit_speed > 40 and self._last_speed_damage_time >= 3 then
        self._last_speed_damage_time = 0
        minetest.sound_play("ju75_collision", {
            --to_player = self.driver_name,
            object = self.object,
            max_hear_distance = 15,
            gain = 1.0,
...
function ju75.checkattachBug(self)
...
function ju75.flap_off(self)
    self._wing_configuration = ju75.wing_angle_of_attack
...
function ju75.flap_operate(self, player)
...
function ju75.door_operate(self, player)
...
function ju75.engine_set_sound_and_animation(self)
...
function ju75.tail(self, longit_speed, pitch)
...
function ju75.flightstep(self)
    ...
    local longit_drag = vector.multiply(hull_direction,longit_speed*
            longit_speed*JU52_LONGIT_DRAG_FACTOR*-1*ju75.sign(longit_speed))
	local later_speed = ju75.dot(velocity,nhdir)
    --minetest.chat_send_all('later_speed: '.. later_speed)
	local later_drag = vector.multiply(nhdir,later_speed*later_speed*
            JU52_LATER_DRAG_FACTOR*-1*ju75.sign(later_speed))
    ...
    local is_attached = ju75.checkAttach(self, player)

	if not is_attached then
        -- for some engine error the player can be detached from the machine, so lets set him attached again
        ju75.checkattachBug(self)
    end
    ...
    if longit_speed then
        local percentage = math.abs(((longit_speed * 100)/(ju75.min_speed + 5))/100)
        ...
        if airutils.adjust_attack_angle_by_speed then
            self._angle_of_attack = airutils.adjust_attack_angle_by_speed(self._angle_of_attack, 2, 5, 45, longit_speed, ju75.ideal_step, self.dtime)
        end
    end
    ...
    if airutils.get_plane_pitch then
        newpitch = airutils.get_plane_pitch(velocity, longit_speed, ju75.min_speed, self._angle_of_attack)
    end
	...
	if math.abs(self._rudder_angle)>1 then
        ...
        local yaw_turn = self.dtime * (turn * ju75.sign(longit_speed) * math.abs(longit_speed/3))
		newyaw = yaw + yaw_turn
	end
    ...
    if is_flying then
        ...
        local prsr = ju75.dot(snormal,nhdir)
        local rollfactor = -90
        local roll_rate = math.rad(10)
        newroll = (prsr*math.rad(rollfactor)) * (later_speed * roll_rate) * ju75.sign(longit_speed)
    ...
    if is_attached or passenger then
        accel, stop = ju75.control(self, self.dtime, hull_direction,
	...
	if self.isinliquid then
        self._engine_running = false
        local bob = ju75.minmax(ju75.dot(accel,hull_direction),0.4)	-- vertical bobbing
        accel.y = accel.y + bob
        local max_pitch = 6
    ...
    if longit_speed > ju75.min_speed / 2 then
        ...
        if is_flying == false and airutils.quadBezier then
            local takeoff_speed = ju75.min_speed * 4  --so first I'll consider the takeoff speed 4x the minimal flight speed
            if longit_speed < takeoff_speed and longit_speed > ju75.min_speed then -- then if the airplane is above the mininam speed and bellow the take off
                ...
                factorized_longit_speed = airutils.quadBezier(scale, ju75.min_speed, longit_speed, longit_speed) --here the magic happens using a bezier curve
        ...
        --now gets the lift!
        new_accel = airutils.getLiftAccel(self, velocity, new_accel, longit_speed, roll, curr_pos, ju75.lift, 20000, 25)
    end
    -- end lift
    ...
    -- sound and animation
    ------------------------------------------------------
    ju75.engine_set_sound_and_animation(self)
    ------------------------------------------------------
    ...
    if longit_speed < (ju75.min_speed / 2) and climb_rate < -3 and is_flying then
        ...
        newpitch = ju75.tail(self, longit_speed, newpitch)
    end

    --minetest.chat_send_all('rate '.. climb_rate)
    local climb_angle = ju75.get_gauge_angle(climb_rate)

    local indicated_speed = longit_speed
    if indicated_speed < 0 then indicated_speed = 0 end
    local speed_angle = ju75.get_gauge_angle(indicated_speed, -45)
    --adjust power indicator
    local power_indicator_angle = ju75.get_gauge_angle(self._power_lever/10) + 90
    local energy_indicator_angle = ju75.get_gauge_angle((JU52_MAX_FUEL - self._energy)/3) - 90

    if is_attached then
        if self._show_hud then
            ju75.update_hud(player, climb_angle, speed_angle, power_indicator_angle, energy_indicator_angle)
        else
            ju75.remove_hud(player)
        end
    end
    ...
    if is_flying == false then --isn't flying?
        ...
        local yaw_turn = self.dtime * math.rad(self._rudder_angle) * turn_rate *
                    ju75.sign(longit_speed) * math.abs(longit_speed/2)
	    newyaw = yaw + yaw_turn
    ...
    if self._wing_configuration == ju75.wing_angle_of_attack and self._flap then
        ju75.flap_on(self)
    end
    if self._wing_configuration ~= ju75.wing_angle_of_attack and self._flap == false then
        ju75.flap_off(self)
    end
    ...
    ju75.consumptionCalc(self, accel)

    --test collision
    ju75.testDamage(self, velocity, curr_pos)

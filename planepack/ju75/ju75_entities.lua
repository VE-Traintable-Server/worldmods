...
ju75.vector_up = vector.new(0, 1, 0)

minetest.register_entity('ju75:engine',{
initial_properties = {
	physical = false,
	...
	mesh = "ju75_propellers.b3d",
    --visual_size = {x = 3, y = 3, z = 3},
	textures = {"ju75_helice.png", "ju75_black.png",
                "ju75_helice.png", "ju75_black.png",
                "ju75_helice.png", "ju75_black.png",
                },
	},
...
-- seat pivot
--
minetest.register_entity('ju75:seat_base',{
initial_properties = {
	physical = false,
	collide_with_objects=false,
	pointable=false,
	visual = "mesh",
	mesh = "ju75_seat_base.b3d",
    textures = {"ju75_black.png",},
	},
...
function ju75.textures_copy()
    local tablecopy = {}
    for k, v in pairs(ju75.textures) do
      tablecopy[k] = v
    end
    return tablecopy
end

minetest.register_entity("ju75:ju75", {
	initial_properties = {
	    physical = true,
    ...
    _flap = false,
    _wing_configuration = ju75.wing_angle_of_attack,
    _passengers_base = {},
    _passengers = {},
    _color = nil,
    _skin = 'ju75_skin_lufthansa.png',
    _inv = nil,
    _inv_id = "",
    _door_closed = true,
    ...
    _change_color = function(self, colstr)
        airutils.paint(self, colstr, ju75.skin_texture)
    end,
	...
	on_activate = function(self, staticdata, dtime_s)
		...
		if not inv then
            airutils.create_inventory(self, ju75.trunk_slots)
		else
		    self.inv = inv
        end
	end,
	...
	on_punch = function(self, puncher, ttime, toolcaps, dir, damage)
        ...
        if is_attached == false then
            if ju75.loadFuel(self, puncher:get_player_name()) then
                return
            end
            ...
            if (item_name == "airutils:repair_tool" or item_name == "default:mese_crystal")
                ...
                if airutils.set_paint(self, puncher, itmstck, ju75.skin_texture) == false then
                    ...
                    self.initial_properties.textures = ju75.textures_copy()
                    airutils.set_paint(self, puncher, itmstck, ju75.skin_texture)
                end
            end
	...
	on_rightclick = function(self, clicker)
        ...
        elseif not self.driver_name then
            if self.owner == name or minetest.check_player_privs(clicker, {protection_bypass=true}) then
                if clicker:get_player_control().aux1 == true then --lets see the inventory
                    airutils.show_vehicle_trunk_formspec(self, clicker, ju75.trunk_slots)
                else
                    if ju75.restricted == "true" and not minetest.check_player_privs(clicker, {flight_licence=true}) then
                        minetest.show_formspec(name, "ju75:flightlicence",
                            "size[4,2]" ..
                            "label[0.0,0.0;Sorry ...]"..
                    ...
                    do 
                        if self._passengers[i] then
                            if self._passengers[i] then
                                ...
                                if passenger then ju75.dettach_pax(self, passenger) end
                            end
                        end
                    end
                    ...
                    ju75.attach(self, clicker)
                    self._command_is_given = false
                end
            else
                ju75.dettach_pax(self, clicker)
                minetest.chat_send_player(name, core.colorize('#ff0000', " >>> You aren't the owner of this Ju 75."))
            end
        ...
        elseif self.driver_name ~= nil or self._autoflymode == true then
            ...
            if player then
                ...
                is_attached = ju75.check_passenger_is_attached(self, name)

...
ju75.ideal_step = 0.02
ju75.rudder_limit = 25
ju75.elevator_limit = 40

dofile(minetest.get_modpath("ju75") .. DIR_DELIM .. "ju75_utilities.lua")

function ju75.powerAdjust(self,dtime,factor,dir,max_power)
    local max = max_power or 100
    local add_factor = factor
    add_factor = add_factor * (dtime/ju75.ideal_step) --adjusting the command speed by dtime
...
function ju75.control(self, dtime, hull_direction, longit_speed, longit_drag,
	...
	if player then
        ...
        if self._engine_running then
            ...
            local engineacc = (self._power_lever * ju75.max_engine_acc) / 100;
            ...
            if ctrl.jump then
                ju75.powerAdjust(self, dtime, factor, 1)
            end
        ...
        ju75.set_pitch(self, pitch_cmd, dtime)

		-- yaw
        local yaw_cmd = 0
        if ctrl.right then yaw_cmd = 1 elseif ctrl.left then yaw_cmd = -1 end
        ju75.set_yaw(self, yaw_cmd, dtime)
    ...
    if longit_speed > 0 then
        if ctrl then
            if ctrl.right or ctrl.left then
                ...
                ju75.rudder_auto_correction(self, longit_speed, dtime)
            end
        else
            ju75.rudder_auto_correction(self, longit_speed, dtime)
        end
        self._elevator_angle = airutils.elevator_auto_correction(self, longit_speed, self.dtime, ju75.max_speed, self._elevator_angle, ju75.elevator_limit, ju75.ideal_step, 100)
    end

    return retval_accel, stop
end

function ju75.set_pitch(self, dir, dtime)
    local pitch_factor = 7
	if dir == -1 then
		self._elevator_angle = math.max(self._elevator_angle-(pitch_factor*dtime),-ju75.elevator_limit)
	elseif dir == 1 then
        if self._angle_of_attack < 0 then pitch_factor = 1 end --lets reduce the command power to avoid accidents
		self._elevator_angle = math.min(self._elevator_angle+(pitch_factor*dtime),ju75.elevator_limit)
	end
end

function ju75.set_yaw(self, dir, dtime)
	...
	if dir == 1 then
		...
		self._rudder_angle = math.max(self._rudder_angle-(yaw_factor*dtime*turn_factor),-ju75.rudder_limit)
	elseif dir == -1 then
        if self._rudder_angle < 0 then turn_factor = correction_factor end
		self._rudder_angle = math.min(self._rudder_angle+(yaw_factor*dtime*turn_factor),ju75.rudder_limit)
	end
end

function ju75.rudder_auto_correction(self, longit_speed, dtime)
    local factor = 1
    if self._rudder_angle > 0 then factor = -1 end
    local correction = (ju75.rudder_limit*(longit_speed/8000)) * factor * (dtime/ju75.ideal_step)
...
function ju75.engineSoundPlay(self)
    --sound
    if self.sound_handle then minetest.sound_stop(self.sound_handle) end
    self.sound_handle = minetest.sound_play({name = "ju75_engine"},
